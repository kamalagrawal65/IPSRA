...
class pages extends Controller {
    ...
    {	
		...
		$this->session->set_userdata('total_resources',$sql);
		
		$last_res= $this->page_model->last_res();
		...
		$this->load->view('submenu',array('ids'=>$ids));
	...
	{	
		echo $this->page_model->update_course();
	}	
	...
	function con_del_pub($master_list_no,$s_no)
	{
		$this->session->set_userdata('master_list_no',$master_list_no);
		$data['results']=$this->page_model->view_res();
		$data['pub_results']=$this->page_model->con_pub($s_no);
		$this->load->view('con_publications',$data);
	}

	function con_del_cou($master_list_no,$s_no)
	{
		$this->session->set_userdata('master_list_no',$master_list_no);
		$data['results']=$this->page_model->view_res();
		$data['course_results']=$this->page_model->con_course($s_no);
		$this->load->view('con_course',$data);
	}
	
	
	function con_del_res($master_list_no)
	{
		$this->session->set_userdata('master_list_no',$master_list_no);
		$data['results']=$this->page_model->view_res();
		$this->load->view('con_del_res',$data);
	}
	
	
	
    function view_cources($master_list_no)
	{	
		$this->session->set_userdata('master_list_no',$master_list_no);
		$data['results']=$this->page_model->view_res();
		$data['course_results']=$this->page_model->view_course();
		$this->load->view('view_courses',$data);
	}	
...
function browse($b_text='%',$start=0)
	{	
		$id = 'c4';
//SET MAin cluster to search		
		if($this->input->post('b_cluster'))
		{
			$b_cluster=$this->input->post('b_cluster');
			if($b_cluster=="All_Resources")
			{
				$b_cluster="";
			}
			$this->session->set_userdata('b_cluster',$b_cluster);
		}
		...
		if(!trim($b_text))
		{
			//echo "Inside browse!!!  Redirecting to Browse09 ".$this->session->userdata('b_cluster');
			redirect('/pages/browse09');
		}
...
function browse09($start=0)
	{	
		$id = 'c4';
//SET BROWSE TEXT
		$results_per_page=10;		
		$this->session->set_userdata('b_text',"");
		$browse_data = $this->page_model->browse09();
		$total_results = $this->page_model->count_browse09();
        $first_result = $start + 1;
        $last_result = min($start + $results_per_page, $total_results);
	...
	function last_upload($start=0,$records=25)
	{
		$b_text=$this->page_model->total_data()-$records;
		$this->session->set_userdata('qsi'," master_list > ".$b_text." ");
		//$this->session->unset_userdata('si');
		$url = '/pages/last_upload/';
	...
	function open_result($b_text)
	{
		...
		$this->session->set_userdata('qsi'," master_list = ".$b_text." ");
		//$this->session->unset_userdata('si');
		$url = '/pages/open_result';
		$this->adsearch2($start,$url);
	}
	
	function new_result($r_name,$r_cat)
	{
		$start=0;
		$this->session->set_userdata('qsi'," Name_of_Resource  = '".$r_name."' and main_clusters = '".$r_cat."'");
		//$this->session->unset_userdata('si');
		$url = '/pages/new_result';
		$this->adsearch2($start,$url);
	}

	
	
	function view_resource($v_text,$start=0)
	{
/*		$this->session->set_userdata('qsi'," main_clusters = '".$v_text."' ");
		$this->session->unset_userdata('si');
		$url = '/pages/adsearch1';
		$this->adsearch2($start,$url);
	*/
		$this->session->set_userdata('b_cluster',$v_text);
		$this->browse();
	}
	
	function view_country($country,$start=0)
	{
		$qsi1 =$this->session->userdata('qsi');
		$this->session->set_userdata('qsi',$qsi1." and Country like '%".$country."%' ");
		$this->session->unset_userdata('si');
		$url = '/pages/adsearch1';
		$this->adsearch2($start,$url);
	}
	
	function view_year($year,$start=0)
	{
		$qsi1 =$this->session->userdata('qsi');
		$this->session->set_userdata('qsi',$qsi1." and Year like '%".$year."%' ");
		$this->session->unset_userdata('si');
		$url = '/pages/adsearch1';
		$this->adsearch2($start,$url);
	}
...
//			$where_query=$where_query." OR MATCH (Name_of_Resource) AGAINST ('".$search_terms3."') > 0 )" ;
	...
	{	
			...
			$yfrom=$this->input->post('yfrom');
			$yto=$this->input->post('yto');
			$match1=$this->input->post('match1');
			
			$si=$s_text1;
			
		if (!trim($s_text1.$s_text2.$s_text3.$s_text4.$s_text5.$yfrom.$yto))
			{
				$errortext='At least one search field must contain text.';
			}
		...
		if(trim($yfrom))
			if (!ctype_digit($yfrom))
					$errortext='Enter Valid Year.';
			elseif ($yfrom<$yearfrom)
					$errortext='Year can not be less than '.$yearfrom.'.';
			
		if(trim($yto))
			if (!ctype_digit($yto))
				$errortext='Enter Valid Year.';
		
		if(trim($yfrom) and ($yto))
			if($yfrom>$yto)
				$errortext='Enter Valid Range.';
		...
		if (trim($errortext))
			{
				$id = 'c2';
				$this->session->set_userdata('error_message',$errortext);
				$this->load->view('siteintial');
				$this->load->view('chooseheader2');
				$this->load->view('mainheader',array('id'=>$id));		
				$this->load->view('adsearch');
		...
		{
			if (trim($search_q))
				$search_q=$search_q." and  ( year >= $yfrom )";
			else 
				$search_q=$search_q." ( year >= $yfrom )";
		}	

		if (trim($yto))
		{
			if (trim($search_q))
				$search_q=$search_q." and  ( year <= $yto )";
			else 
				$search_q=$search_q." ( year <= $yto )";
		}	
	...
	{	
		...
		if (trim($s_text1))
			{$search_q=$this->build_query2($search_q, $match1, $sfild1, $s_text1);}
		if (trim($s_text2))
			{$search_q=$this->build_query2($search_q, $match1, $sfild2, $s_text2);}
		if (trim($s_text3))
			{$search_q=$this->build_query2($search_q, $match1, $sfild3, $s_text3);}
		if (trim($s_text4))
			{$search_q=$this->build_query2($search_q, $match1, $sfild4, $s_text4);}
		if (trim($s_text5))
			{$search_q=$this->build_query2($search_q, $match1, $sfild5, $s_text5);}
		...
		$pub_results = $this->page_model->search_pub($search_q);
		$this->load->view('Show_Publications', array('pub_results' => $pub_results));

	}
	
	function build_query2($search_q, $opr1, $field1, $text1)
	{
		if (trim($search_q))
		{
			$search_q=$search_q." ".$opr1." ";
	...
	{	
		...
		if (trim($s_text5))
		 ...
		 $cou_results = $this->page_model->search_cou($search_q);
		$this->load->view('show_courses', array('course_results' => $cou_results));

	}
	
	function adsearch2($start=0,$url='/pages/adsearch1')
	{	
		$results_per_page = $this->results_per_page();
